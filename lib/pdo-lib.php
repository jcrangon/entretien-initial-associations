...
function getPDO($dbdata){
	...
	if($log){$logger->info("Dans function getPDO()","");}
	$dsn=$dbdata["driver"].":host=".$dbdata["serveur"].";port=".$dbdata["port"].";dbname=".$dbdata["base"].";charset=".$dbdata["charset"];
	$user=$dbdata["user"];
	$pwd=$dbdata["pass"];
	$options=$dbdata["options"];
	try{
		if($pdo= new PDO($dsn,$user,$pwd,$options) ){
			if($log){$logger->info("Connexion!! retour : PDO Object","");}
			return $pdo;
		}
		else{
			if($log){$logger->error("Erreur Connexion!! retour : -1","");}
			return -1;
		}
	}
	catch(PDOexception $e){
		if($log){$logger->error("exception Fatale!! retour : false / PDOException:",$e->getMessage());}
		return false;
	}
}
